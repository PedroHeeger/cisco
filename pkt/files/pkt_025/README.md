...
- f. Clique no quadrado na coluna Tipo para o primeiro evento, que deve ser ICMPv6. Uma vez que a mensagem começa com este evento, existe apenas uma PDU de saída. Na guia Modelo OSI, qual é o Tipo de Mensagem listado para ICMPv6? Observe que não há endereçamento de Camada 2. Clique no botão Próxima Camada >> para obter uma explicação sobre o processo ND (Descoberta de Vizinhos).
  - O tipo de mensagem listado é ICMPv6 Echo Request (128), usado para iniciar o teste de conectividade (ping). Esse pacote faz parte do Internet Control Message Protocol version 6.
...
- h. Volte para a lista de eventos. Selecione o primeiro evento NDP no SwitchA. Existe alguma diferença entre as Camadas Dentro e Fora da Camada 2?
  - Não. As informações da Camada 2 permanecem inalteradas, pois o switch apenas encaminha o quadro Ethernet sem modificar os endereços MAC de origem e de destino.
- i. Selecione o primeiro evento NDP no PCA2. Clique na guia Detalhes da PDU de Saída. Quais endereços são exibidos para o seguinte? Observação: os endereços nos campos podem ser quebrados, ajuste o tamanho da janela da PDU para facilitar a leitura das informações de endereço.  
  - ADDR DEST Ethernet II: 3333:FF00:000B (MAC multicast usado pelo IPv6 Neighbor Discovery) / 0001.427E.E8ED (endereço MAC do PCA1)
  - ADDR SRC Ethernet II: 0040.0BD2.243E (MAC do PCA2)
  - IPv6 SRC IP: 2001:DB8:ACAD:1::B (endereço IPv6 do PCA2)
  - IPv6 DST IP: FF02::1:FF00:B (endereço multicast de nó solicitado usado pelo Neighbor Discovery Protocol) / 2001:db8:acad:1::a (endereço de IPv6 do PCA1)
  - Embora, em implementações reais do IPv6, a Neighbor Advertisement normalmente seja enviada por unicast ao solicitante, na simulação do Packet Tracer ela é exibida sendo enviada em multicast, permitindo que o roteador aprenda o endereço IPv6 e o endereço MAC do PCA2.
...
- m. Clique no último evento ICMPv6 em PCA1. Observe que esta é a última comunicação listada. O que é o tipo de mensagem de eco ICMPv6?
  - O tipo de mensagem é Echo Reply (129), que é a resposta ao Echo Request enviado anteriormente dentro do Internet Control Message Protocol version 6.
...
A imagem 02 apresenta as informações da PDU de entrada correspondente à mensagem ICMP Echo Reply, recebida pelo PCA1 após a conclusão do processo de Neighbor Discovery, confirmando a resposta ao teste de conectividade (ping).
...
- a. Clique no botão Reset Simulation para limpar os eventos anteriores. Exibir e limpar todas as entradas na tabela de dispositivos vizinhos IPv6 como foi feito na Parte I.
...
- o. Clique no único evento PCB1 na nova lista. A que corresponde o endereço MAC de destino?
  - Corresponde ao MAC da interface do roteador conectada à rede do PCB1 (G0/0/1), que atua como gateway padrão para encaminhar o tráfego para outras redes.
...
| Dispositivo                  |  MAC            | IP - GUA / Multicast  | IP - LLA                 |
|:----------------------------:|:---------------:|:---------------------:|:------------------------:|
| **PCA1**                     | 0001.427E.E8ED  | 2001:db8:acad:1::A/64 | FE80::201:42FF:FE7E:E8ED |
| **PCA2**                     | 0040.0BD2.243E  | 2001:db8:acad:1::B/64 | FE80::240:BFF:FED2:243E  | 
| **RTA-G0/0/0**               | 0001.961D.6301  | 2001:db8:acad:1::1/64 | FE80::1                  |
| **RTA-G0/0/1**               | 0001.961D.6302  | 2001:db8:acad:2::1/64 | FE80::1                  |
| **PCB1**                     | 0060.2F68.9E91  | 2001:db8:acad:2::A/64 | FE80::260:2FFF:FE68:9E91 |
| **Solicited-Node Multicast** | 3333.FF00.000B  | FF02::1:FF00:B        | PCA2                     |
| **Solicited-Node Multicast** | 3333.FF00.0001  | FF02::1:FF00:1        | RTA G0/0/0 e RTA G0/0/1  |
| **Solicited-Node Multicast** | 3333.FF00.000A  | FF02::1:FF00:A        | PCA1 e PCB1              |
...
- b. Clique em RTA e selecione a guia CLI. No prompt do roteador, digite o comando show ipv6 neighbors. Quantos endereços estão listados?
  - Cinco endereços IPv6, correspondentes aos endereços globais dos hosts e aos respectivos endereços locais de enlace (link-local) dos dispositivos vizinhos.
- b. A que dispositivos esses endereços estão associados?
  - Os endereços estão associados ao PCA1, PCA2 e PCB1, incluindo seus respectivos endereços IPv6 globais e alguns endereços locais de enlace (link-local).
- b. Há alguma entrada para PCA2 listada (por que ou por que não)?
  - Sim. Embora não tenha havido comunicação direta entre o PCA2 e o roteador, o PCA2 respondeu às mensagens de Neighbor Discovery em multicast, permitindo que o roteador aprendesse seu endereço IPv6 e o respectivo endereço MAC e os registrasse na tabela de vizinhos.
- c. Ping PCA2 a partir do roteador.
  - `ping 2001:DB8:ACAD:1::B`.
- d. Emita o comando show ipv6 neighbours. Há entradas para o PCA2?
  - Sim. A entrada referente ao PCA2 permaneceu na tabela de vizinhos, pois o roteador já havia aprendido seu endereço IPv6 e o respectivo endereço MAC durante o processo de Neighbor Discovery.

A imagem 03 apresenta a tabela de vizinhos IPv6 do roteador, contendo os endereços IPv6 e os respectivos endereços MAC dos dispositivos aprendidos por meio do Neighbor Discovery.
