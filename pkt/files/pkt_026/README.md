...
- b. Insira o comando `show running-config`. Qual é o nome de host do roteador?
...
- c. Exiba o conteúdo atual da NVRAM (`show startup-config`). Por que o roteador responde com a mensagem startup-config não está presente?
...
- c. Se você não for solicitado uma senha antes de acessar o prompt do usuário EXEC, qual comando da linha do console você esqueceu de configurar?
  - O comando seria `enable password cisco`.
...
- a. Examine o conteúdo do flash usando o comando `show flash`. Quantos arquivos estão armazenados na memória flash no momento?
...
- a. Quais desses arquivos você diria que é a imagem IOS?
  - O arquivo que corresponde à imagem IOS é `c1900-universalk9-mz.SPA.151-4.M4.bin`.
...
- b. Salve o arquivo de configuração inicial na memória flash usando os seguintes comandos: `copy startup-config flash`. O roteador solicita que você armazene o arquivo em flash usando o nome entre colchetes. Se a resposta for sim, pressione ENTER; caso contrário, digite um nome adequado e pressione ENTER.
